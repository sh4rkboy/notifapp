...
$(document).on('pageinit','#page1',function(){
     ...
     $('.back').click(function(){
        $.mobile.navigate("#page1",{transition: "slide",direction: "reverse",info: "info goes here"});
    });
     $('#page2b').click(function(){
        $.mobile.navigate("#page2",{transition: "slide",info: "info goes here"});
        createDB();
    });
  ...
  if(($("#DatePrint").text() != "") && ($("#TimePrint").text() != "") ){
      if($("#Task_title").val() != ""){
        cordova.plugins.notification.local.schedule({
            ...
            text: 'Until '+DateTime.hour+":"+DateTime.minute,
        ...
        insertDB();
          ...
          window.alert("Set a Title for your recordatory");
      }
    }
    else{
        window.alert("Set a Date and a Time for your recordatory");
    }
       ...
       var counter = rs.rows.item(0).mycount;
        ...
        for(i = 1; i <= counter;i++){
           db.executeSql('SELECT id, description, date , count(*) AS mycount FROM appointments WHERE id='+i, [], function(res) {
                        ...
                        $("#ListHandler").append(
                                                 '<li id="Elem'+res.rows.item(0).id+'" img src="img/x.png" style="margin: 10px 10px 10px 10px; border:none;" class="ui-body-a">'+
                                                    '<div>'+
